...
test('Login', async () => {
	...
	await page.locator('[name="auth[password]"]').fill('wrong');
	await button(page, 'Login').click();
	await expect(page.locator('body')).toContainText('Access denied for user');
	await page.locator('#username').fill('ODBC');
...
test('Create database', async () => {
	await goto(page, '/adminer/?username=ODBC');
...
test('Create index', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&table=interprets');
...
test('Check constraints', async () => {
	...
	await button(page, 'Save').click();
	await expect(page.locator('body')).toContainText("Check constraint 'albums_interpret_check' is violated.");
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&check=albums&name=albums_interpret_check');
...
test('Create view', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&view=');
...
test('Invalid table', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&table=invalid');
	await expect(page.locator('body')).toContainText('No tables.');
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&create=invalid');
	await expect(page.locator('body')).toContainText('No tables.');
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&select=invalid');
	await expect(page.locator('body')).toContainText('Unable to select the table:');
});

test('Schema', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&schema=');
	await expect(page.locator('body')).toContainText('Permanent link');
});

test('Insert', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&edit=interprets');
	...
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&edit=albums');
...
test('Pagination', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&select=albums&limit=1');
	...
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&select=albums&limit=1&page=last');
	await expect(page.locator('body')).toContainText('Black and White');
...
test('Search in tables with special types', async () => {
	const sql = "CREATE TABLE types (id int PRIMARY KEY, b bit(3), bin binary(4), g point, j json, t text);"
		+ " INSERT INTO types VALUES (1, b'101', 'abc3', ST_GeomFromText('POINT(3 4)'), '{\"a\": 3}', 'abc3')";
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&sql=' + encodeURIComponent(sql));
	await button(page, 'Execute').click();
	await expect(page.locator('body')).toContainText('Query executed OK');
	await goto(page, '/adminer/?username=ODBC&db=adminer_test');
	for (const [op, query] of [['LIKE %%', 'abc'], ['LIKE %%', '3'], ['=', 'abc3'], ['REGEXP', 'abc']]) {
	...
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&sql=' + encodeURIComponent('DROP TABLE types'));
	await button(page, 'Execute').click();
});

test('Update', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&edit=albums&where[id]=2');
...
test('Modify', async () => {
	...
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&select=albums&text_length=5');
...
test('Bulk table operations', async () => {
	...
	await goto(page, '/adminer/?username=ODBC&db=adminer_test2');
	...
	await goto(page, '/adminer/?username=ODBC&sql=' + encodeURIComponent('DROP DATABASE adminer_test2'));
	await button(page, 'Execute').click();
	await expect(page.locator('body')).toContainText('Query executed OK');
});

test('SQL file import', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&import=');
	...
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&sql=' + encodeURIComponent('SELECT * FROM interprets'));
	...
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&sql=' + encodeURIComponent("DELETE FROM interprets WHERE name = 'Karel Gott'"));
	await button(page, 'Execute').click();
});

test('Privileges', async () => {
	await goto(page, '/adminer/?username=ODBC&user=');
...
test('Process list', async () => {
	await goto(page, '/adminer/?username=ODBC&processlist=');
	await expect(page.locator('body')).toContainText('SHOW FULL PROCESSLIST');
});

test('Export', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&dump=');
...
test('Events', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&event=');
...
test('Procedures', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&procedure=');
...
test('Generated columns', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&create=');
...
test('Variables', async () => {
	await goto(page, '/adminer/?username=ODBC&variables=');
	await expect(page.locator('body')).toContainText('basedir');
	await goto(page, '/adminer/?username=ODBC&status=');
	await expect(page.locator('body')).toContainText('Uptime');
});

test('History', async () => {
	await goto(page, '/adminer/?username=ODBC&sql=');
	await expect(page.locator('body')).toContainText('DROP DATABASE IF EXISTS adminer_test');
});

test('Warnings', async () => {
	await goto(page, '/adminer/?username=ODBC&db=adminer_test&select=albums');
	await page.locator('[id="th[interpret]"]').hover(); // displays the column menu
	await page.locator("//th[@id='th[interpret]']/span/a[2]").click();
	await page.locator('[name="where[0][val]"]').fill('1.2.3');
	await page.locator('#form').evaluate(form => form.submit());
	await link(page, 'Warnings').click();
	await expect(page.locator("//div[@id='warnings']/div/table/tbody/tr/td[3]")).toHaveText("Truncated incorrect DOUBLE value: '1.2.3'");
});

test('Editor', async () => {
	await goto(page, '/editor/example.php');
	await page.locator('[name="lang"]').selectOption({label: 'English'}); // submits the form
...
test('Logout', async () => {
	...
	await page.locator('select[name="lang"]').selectOption({label: 'Čeština'}); // submits the form
	await expect(page.locator('body')).toContainText('Přihlásit se');
});
